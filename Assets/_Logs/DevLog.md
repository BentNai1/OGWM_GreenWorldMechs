...
1. UI to allow each part to change to another part from a list.
	a. [x] ~~(?) Obviously primarily this UI is for play mode, but should this UI work for edit mode too, or should a separate editmode UI be made?~~
		i. **One UI, different entry points - buttons on the inspector to enable/disable the singular UI**
	b. [ ] A ui container that either shows a list of the current parts on the mech, or when editing a slot, shows the list of available parts for that slot.
	c. [ ] a script component that goes with each slot. (? Mech Puppet Slot Controller maybe already does everything I need)
	d. [ ] new sub-list struct of "available parts"
	...
	e. [ ] dropdown menu to select parts, should have the name, slot(s), and a picture of the part.
...
2. Sprite property for parts
	a. Separate garage preview for current full sprite (built from parts)
